...
`MoonContext MCP` is an MCP (Model Context Protocol) server that gives AI coding
agents two things they lack when writing MoonBit:
...
2. **Project context** — the relevant parts of a MoonBit project, packed to fit
   a token budget.
...
- Tests must not require live network access. The mooncakes.io client is tested
  against **recorded JSON fixtures** so the suite stays reproducible offline.
...
- All network access must be cache-backed so the acceptance reviewer can run the
  project offline.
