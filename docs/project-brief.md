...
- **第三方依赖**：协议层计划依赖社区包 `marianoguerra/mcp`（零依赖，含 JSON-RPC codec 与 MCP 类型）。这是第三方依赖，而非移植或改写；其名称、来源与许可证将在 README 的 `Dependencies & Attribution` 小节如实标注。若该包 API 不满足需求，则自行实现最小 JSON-RPC 2.0 层。
